...
namespace :an_axe do 
	desc "Add database tables for an_axe"
	task :bootstrap_db => :environment do
    migration_path = Rails.root + "/db/migrate/#{Time.now.utc.strftime("%Y%m%d%H%M%S")}_create_savage_tables.rb"
    an_axe_migration = File.join(SAVAGE_BEAST_BASE_DIR, 'db/migrate/', '001_create_savage_tables.rb')
    FileUtils.cp(an_axe_migration, File.expand_path(migration_path))
		puts "Savage Beast migration copied to db/migrate.  Run rake db:migrate to complete the db bootstrap process."
	end

  desc "[Deprecated] Copy the stylesheets and Javascripts used natively by an_axe into host app's public directory"
  task :bootstrap_assets => :environment do
    puts "Please run\n  rails generate an_axe:install"
  end

  desc "Adds an_axes gem requirements to enviroment.rb"
  ...
  desc "Add includes for an_axe in application_helper and models/user"
